#!/usr/bin/env python3
"""
Place an outbound call via the Plivo Voice API
"""
import argparse
import base64
import json
...
def make_call(to, answer_url, src=None, auth_id=None, auth_token=None):
    """Place an outbound call via the Plivo Voice API"""
    ...
    payload = json.dumps({'from': src, 'to': to, 'answer_url': answer_url}).encode('utf-8')
    ...
    try:
        ...
        if status == 201:
            ...
            print(f"✓ Call fired to {to} (request_uuid: {request_uuid})")
            return True
...
def main():
    parser = argparse.ArgumentParser(description='Place an outbound call via the Plivo Voice API')
    parser.add_argument('--to', required=True, help='Destination number in E.164')
    parser.add_argument('--answer-url', dest='answer_url', required=True, help='Publicly reachable URL returning call-flow XML')
    parser.add_argument('--from', dest='src', help='Caller ID (default: PLIVO_SRC env var)')
